...
- **Reschedule-then-backstop**: a guard commits _before_ the Alpaca touch, so
  ...
  `PlaceHedge` claims the position (`PositionCommand::
  PlaceOffChainOrder`)
  before the broker call, so a rescheduled successor hits
  `PositionError::PendingExecution` and calls `recover_pending_poll_status`
  instead of re-placing the order. For both, the reschedule's value is narrower
  than for a true-retry job: it still prevents burning the terminal retry budget
  and still frees the worker, but actual completion is delegated to the existing
  backstop (`CheckPositions`/poll-status recovery), not the reschedule itself,
  and the streak structurally caps at 1 (a guard that already committed cannot
  un-commit on a later attempt).
